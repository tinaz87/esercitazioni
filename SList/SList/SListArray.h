...
namespace sla{	
	...
	template<typename T,typename Allocator>
	bool operator==(SListArray<T,Allocator>& l_val,SListArray<T,Allocator>& r_val){

		if(l_val.size() != r_val.size())
			return false;

		SListArray<T,Allocator>::iterator left_it = l_val.begin();
		SListArray<T,Allocator>::iterator right_it = r_val.begin();
